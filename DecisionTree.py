...
# import data
data = pd.read_csv("HousingData.csv", index_col = 'Id')


# categories being used to predict SalePrice (not all are used)
data = data[['MSSubClass', 'MSZoning', 'LotFrontage', 'LotArea', 'Street', 'Alley', 'LotShape', 'LandContour',
...
# score predicted output of our model on test data
y_predict = model.predict(x_test)

# model's predicted SalePrice
...
# x_train = categories for train group
# y_train = SalePrice for train group  (80%)
# x_test = categories for test group
# y_test = SalePrice of test group   (20%)

# prints accuracy score, comparing prediction vs test data (only accurate if exact match)
# print 'Accuracy score: ', accuracy_score(y_test, y_predict)

# figure out accuracy interval based on real SalePrice
# convert sets to lists to increase functionality
real_price_list = list(y_test)
predicted_price_list = list(y_predict)
# print y_test_list[0]
# print y_predict_list[0]
...
for i in range(1, len(real_price_list)):
    if (1 - tolerance) * real_price_list[i] <= predicted_price_list[i] and (1 + tolerance) * real_price_list[i] >= predicted_price_list[i]:
        # print 'Prices in accuracy interval: ', y_predict_list[i]
        accuracy_counter += 1
...
# plot scatter plot of True SalePrice vs. Predicted SalePrice
plt.scatter(range(len(y_test)), real_price_list, label = 'True SalePrice')
plt.scatter(range(len(y_predict)), predicted_price_list, label = 'Predicted SalePrice')
plt.xlabel('House Number')
plt.ylabel('Sale Price of House (in $)')
plt.title('Performance of Decision Tree Classifier')
plt.legend()
plt.show()
